...
class ClearCore:
    ...
    async def go_for(self, rpm: float, revolutions: float, *, extra: Optional[Dict[str, Any]] = None,
        ...
        resp = await self.drive_write('RM', str(revolutions))
        return resp

    async def go_to(self, rpm: float, position_revolutions: float, *, extra: Optional[Dict[str, Any]] = None,
                    timeout: Optional[float] = None, **kwargs):
        await self.change_speed(rpm)
        resp = await self.drive_write('AM', str(position_revolutions))
        return resp

    async def jog(self, rpm: float):
        rpm = await check_speed(rpm)
        resp = await self.drive_write('JG', str(rpm))
        return resp
    ...
    async def change_speed(self, rpm: float, *, extra: Optional[Dict[str, Any]] = None, timeout: Optional[float] = None,
                           **kwargs):
        rpm = await check_speed(rpm)
        resp = await self.drive_write('SV', str(rpm))
        return resp
